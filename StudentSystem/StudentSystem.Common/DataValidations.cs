namespace StudentSystem.Common
{
    public static class DataValidations
    {
        public const int USER_NAME_MAX_LENGTH = 40;
        public const int USER_NAME_MIN_LENGTH = 5;

        public const int USER_PASSWORD_MAX_LENGTH = 40;
        public const int USER_PASSWORD_MIN_LENGTH = 5;

        public const int USER_FIRST_NAME_MAX_LENGTH = 30;
        public const int USER_FIRST_NAME_MIN_LENGTH = 5;

        public const int USER_MIDDLE_NAME_MAX_LENGTH = 30;
        public const int USER_MIDDLE_NAME_MIN_LENGTH = 5;

        public const int USER_LAST_NAME_MAX_LENGTH = 30;
        public const int USER_LAST_NAME_MIN_LENGTH = 5;

        public const int TOWN_NAME_MAX_LENGTH = 50;
        public const int TOWN_NAME_MIN_LENGTH = 5;

        public const int DEPARTMENT_NAME_MAX_LENGTH = 100;
        public const int DEPARTMENT_NAME_MIN_LENGTH = 5;

        public const int COURSE_NAME_MAX_LENGTH = 40;
        public const int COURSE_NAME_MIN_LENGTH = 5;
    }
}
